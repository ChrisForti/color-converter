...
export function ProxyPreview() {
  ...
  const [selectedMapping, setSelectedMapping] = useState<string>('blue-to-purple');
  const [customMappings, setCustomMappings] = useState<Record<string, ColorMappingConfig>>({});
  ...
  const injectCssIntoIframe = (iframe: HTMLIFrameElement, forceRefresh = false) => {
    try {
      ...
      if (iframeDoc) {
        ...
        existingStyles.forEach(style => {
          console.log('🗑️ Removing existing style:', style.id || style.className);
          style.remove();
        });
        ...
        styleElement.textContent = `
          /* Tailwind Color Converter Overrides - ${activeMapping?.name || 'Unknown'} */
        ...
        if (iframeDoc.head) {
          ...
          console.log(`✅ Injected CSS (ID: ${injectionId}) for mapping: ${activeMapping?.name}`);
          console.log(`📋 CSS Rules count: ${generateCssOverrides().split('\n').length}`);
          return true;
        } else {
      ...
      console.warn('❌ Cannot inject CSS into iframe due to CORS restrictions:', e);
      return false;
    }
    return false;
  };
  ...
  const getFallbackFilter = (mapping: ColorMappingConfig): string => {
    ...
    if (mappings.some(([from, to]) => from.includes('blue') && to.includes('purple'))) {
      return 'hue-rotate(60deg) saturate(1.1);';
    } else if (mappings.some(([from, to]) => from.includes('blue') && to.includes('green'))) {
      return 'hue-rotate(120deg) saturate(1.2);';
    } else if (mappings.some(([from, to]) => from.includes('gray') && to.includes('slate'))) {
      return 'contrast(1.1) brightness(0.95);';
    } else if (mappings.some(([from, to]) => from.includes('blue') && to.includes('red'))) {
      return 'hue-rotate(-60deg) saturate(1.1);';
    } else {
  ...
  useEffect(() => {
    ...
    if (loadedUrl && selectedMapping && allMappings[selectedMapping]) {
      ...
      console.log('🔄 Using fresh mapping:', freshMapping.name, 'for key:', selectedMapping);
      ...
      const attemptInjection = (attempt = 1) => {
        ...
        if (iframe) {
          console.log(`🎨 Attempt ${attempt}: Switching to mapping: ${freshMapping.name}`);
          
          // Try direct CSS injection first
          const success = injectCssIntoIframe(iframe, true);
          
          if (success) {
            console.log(`✅ Successfully applied mapping: ${freshMapping.name}`);
          } else if (attempt < 3) {
            console.log(`⚠️ Attempt ${attempt} failed, retrying...`);
            setTimeout(() => attemptInjection(attempt + 1), 500);
          } else {
            // All direct injection attempts failed, use fallback
            console.log('🔄 All direct injection attempts failed, switching to fallback mode...');
            const fallbackSuccess = applyFallbackColorMapping();
            if (fallbackSuccess) {
              console.log(`✅ Fallback color mapping applied: ${freshMapping.name}`);
            } else {
  ...
  const generateCssOverrides = (): string => {
    ...
    console.log('🎨 Generating CSS for mapping:', activeMapping.name, 'with', Object.keys(activeMapping.mappings).length, 'rules');
    ...
    <div className="max-w-6xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-gray-600 mb-6">
          Enter a website URL to preview it with different color schemes. 
          Note: Some sites may block embedding due to security policies.
        </p>

        {/* Example URLs */}
        <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <p className="text-sm font-medium text-blue-900 mb-2">Try these example sites:</p>
          <div className="flex flex-wrap gap-2">
            {[
              ...
              <button
                ...
                className="text-xs px-2 py-1 bg-white border border-blue-300 rounded text-blue-700 hover:bg-blue-100 transition-colors"
              >
        ...
        <div className="mb-6 p-4 bg-gray-50 rounded-lg border">
          <div className="flex items-center justify-between mb-4">
            ...
            {currentMapping && (
              <span className="text-xs text-gray-500">
                {Object.keys(allMappings[selectedMapping]?.mappings || {}).length} color{Object.keys(allMappings[selectedMapping]?.mappings || {}).length !== 1 ? 's' : ''} mapped
              </span>
            )}
          </div>
          ...
          <select
            ...
            onChange={(e) => {
              console.log('🔄 Mapping changed from', selectedMapping, 'to', e.target.value);
          ...
          {allMappings[selectedMapping] && (
            <div className="mt-3 p-3 bg-white rounded border">
              <h4 className="text-sm font-medium text-gray-700 mb-2">Active Color Mappings:</h4>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs mb-3">
                {Object.entries(allMappings[selectedMapping].mappings).map(([from, to]) => (
                  <div key={`${from}-${to}`} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                    <span className={`px-2 py-1 rounded text-white bg-${from.replace('-', '-')}`} 
                          style={{ backgroundColor: from.includes('blue') ? '#3b82f6' : 
                                                  from.includes('red') ? '#ef4444' : 
                                                  from.includes('green') ? '#10b981' : 
                                                  from.includes('purple') ? '#8b5cf6' : '#6b7280' }}>
                      {from}
                    </span>
                    <span className="mx-2 text-gray-400">→</span>
                    <span className={`px-2 py-1 rounded text-white bg-${to.replace('-', '-')}`}
                          style={{ backgroundColor: to.includes('blue') ? '#3b82f6' : 
                                                  to.includes('red') ? '#ef4444' : 
                                                  to.includes('green') ? '#10b981' : 
                                                  to.includes('purple') ? '#8b5cf6' : '#6b7280' }}>
                      {to}
                    </span>
                  </div>
                ))}
              ...
              <details className="mt-2">
                ...
                <div className="mt-2 p-2 bg-gray-100 rounded text-xs overflow-x-auto">
                  <div className="mb-2 p-1 bg-yellow-100 text-yellow-800 rounded">
                    ...
                    Selected: {selectedMapping}<br/>
                    Active Mapping: {allMappings[selectedMapping]?.name}<br/>
                    Last Update: {new Date(lastMappingUpdate).toLocaleTimeString()}
                  </div>
                ...
                onClick={() => {
                  ...
                  if (iframe) {
                    ...
                    if (success) {
                      ...
                      console.log('🔄 Manual refresh failed, using fallback...');
                      const fallbackSuccess = applyFallbackColorMapping();
                      if (fallbackSuccess) {
                        console.log('🔄 Manual fallback refresh successful');
                      } else {
                        console.warn('❌ Manual refresh failed - both methods unsuccessful');
                      }
                    }
                  }
                }}
              ...
              <>
                <iframe
                  ...
                  onLoad={(e) => {
                    ...
                    const existingParentStyle = parentDoc.getElementById('proxy-color-overrides');
                  ...
                  onError={() => {
                    setError('Failed to load website. This site may not allow embedding.');
                    setIsBlocked(true);
                  }}
                />
                ...
                <div className="absolute bottom-4 right-4 bg-black bg-opacity-75 text-white p-3 rounded-lg text-sm max-w-xs">
                  <div className="flex items-center gap-2 mb-2">
                    <span className={`w-2 h-2 rounded-full ${Date.now() - lastMappingUpdate < 2000 ? 'bg-yellow-400 animate-pulse' : 'bg-green-400 animate-pulse'}`}></span>
                    <p className="font-semibold">
                      {Date.now() - lastMappingUpdate < 2000 ? 'Updating Colors...' : 'Color Override Active'}
                    </p>
                  </div>
                  <p className="text-xs opacity-90 mb-2">
                    {Object.keys(allMappings[selectedMapping]?.mappings || {}).length} color mappings applied
                  </p>
                  <p className="text-xs opacity-75">
                    Mapping: {allMappings[selectedMapping]?.name}
                  </p>
                  <p className="text-xs opacity-75 mt-1">
                    {document.getElementById('proxy-fallback-styles') ? 'Fallback mode active' : 'Direct CSS injection'}
                  </p>
                  <p className="text-xs opacity-75 mt-1">
        ...
        <div className="border border-yellow-200 rounded-lg overflow-hidden bg-yellow-50">
          <div className="p-6 text-center">
            ...
            <div className="bg-white p-4 rounded-lg border border-yellow-200">
              <h4 className="font-medium text-yellow-900 mb-2">Try These Alternatives:</h4>
              <div className="text-sm text-yellow-800 space-y-2">
                <p>• Use the Interactive Editor mode to test your HTML/CSS directly</p>
                <p>• Try one of the working example URLs above</p>
                <p>• Look for simpler websites that allow embedding</p>
                <p>• Copy your CSS overrides and test them manually in browser dev tools</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="mt-8 bg-blue-50 border border-blue-200 rounded-lg p-6">
        ...
        <div className="grid md:grid-cols-2 gap-4 text-blue-800 text-sm">
          <div>
            ...
            <ul className="space-y-1">
              ...
              <li>• Select a color mapping to apply Tailwind color overrides</li>
              <li>• Use the example URLs for sites that work well</li>
              <li>• Try the refresh button if colors don't update immediately</li>
            </ul>
          </div>
          <div>
            ...
            <ul className="space-y-1">
              ...
              <li>• Some sites may block embedding due to X-Frame-Options headers</li>
              <li>• CORS policies may prevent color injection on external sites</li>
        ...
        <div className="mt-4 p-3 bg-blue-100 rounded-lg">
          ...
          <p className="text-blue-800 text-sm">
            For the best experience, try simple websites, documentation sites, or development tools. 
            Many social media and commercial sites block embedding for security reasons.
          </p>
        </div>
      </div>
    </div>
