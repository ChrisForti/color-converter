 ...
 */
function extractColorInfo(className: string): { prefix: string, color: string, shade?: string } | null {
...
export function mapColorClass(
  ...
  if (targetColor && TAILWIND_COLORS.has(targetColor)) {
    ...
    return shade ? `${prefix}${targetColor}-${shade}` : `${prefix}${targetColor}`;
  }
 ...
 */
export function createColorMapping(colorMappings: Record<string, string>): ColorMapping {
  ...
  for (const [from, to] of Object.entries(colorMappings)) {
    ...
    if (!isValidTailwindColor(to)) {
      console.warn(`Invalid target color "${to}" for mapping "${from}" -> "${to}" - skipping`);
      continue;
    }
