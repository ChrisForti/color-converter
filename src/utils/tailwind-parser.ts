...
export function parseColorClasses(input: string): TailwindColorClass[] {
  ...
  classMatches.forEach(match => {
    const classContent = match.replace(/class(?:Name)?=["'`]/, '').replace(/["'`]$/, '');
...
export function parseColorClass(className: string): TailwindColorClass | null {
  ...
  for (const property of COLOR_PROPERTIES) {
    ...
    const specialColors = ['white', 'black', 'transparent', 'current', 'inherit'];
 ...
 */
export function getUniqueColorClasses(classes: TailwindColorClass[]): TailwindColorClass[] {
